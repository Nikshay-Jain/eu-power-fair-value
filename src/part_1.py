...
import os, shutil, zipfile, warnings, logging
...
def main():
    ...
    df_merged['_sort_dt'] = pd.to_datetime(
        df_merged['MTU (UTC)'].str.split(' - ').str[0], 
        dayfirst=True, 
        utc=True
    )
    df_merged = df_merged.sort_values("_sort_dt").drop(columns=['_sort_dt']).reset_index(drop=True)

    report = generate_qa_report(df_merged, mtu_col="MTU (UTC)", output_prefix="qa_report")
    logging.info("\nQA Report Summary:\n%s\n", report["summary_text"])

    df_imputed = impute_energy_ts(df_merged, "MTU (UTC)")
    output_path = os.path.join("data", "cleaned_energy_data.csv")
    df_imputed.to_csv(output_path, index=False)
    logging.info("PART 1 COMPLETE: Saved cleaned data to %s", output_path)
